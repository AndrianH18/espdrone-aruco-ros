...
UTILS_PACKAGE_NAME = "espdrone_utils"
...
def get_espdrone_launch_config(
    ...
    espdrone_keyboard_launch_file = [
        UTILS_PACKAGE_NAME,
        "teleop_hover_waypoints_keyboard.launch",
    ]
    bringup_package_path = rospkg.RosPack().get_path(BRINGUP_PACKAGE_NAME)

    for drone_name in drone_names:
        drone_config_path = os.path.join(
            bringup_package_path, "config", "espdrone", f"{drone_name}.yaml"
        )
        drone_config = read_yaml_config(drone_config_path)

        camera_info_full_path = os.path.join(
            bringup_package_path,
            "config",
            "espdrone",
            "camera_calib",
            drone_config["camera_info_file"],
        )
        aruco_map_full_path = os.path.join(
            bringup_package_path,
            "config",
            "environment",
            "aruco_maps",
            env_config["aruco_map_config_file"],
        )
        ...
        if keyboard:
            keyboard_launch = espdrone_keyboard_launch_file.copy()
            ...
            launch_list.append((keyboard_launch_file, keyboard_launch_args))
...
def main():
    parser = argparse.ArgumentParser(
        description="Launch multiple ESP-drones inside one environment/map with ArUco markers"
    )
    parser.add_argument(
        "--drones",
        "-d",
    ...
    group.add_argument(
        ...
        "--keyboard",
        action="store_true",
        help="Control and command the drone using keyboard keyboard",
    )
    ...
    env_config_path = os.path.join(
        bringup_package_path, "config", "environment", f"{args.env}.yaml"
    )
    ...
    launch_list.extend(
        get_espdrone_launch_config(args.drones, env_config, args.keyboard)
    )
